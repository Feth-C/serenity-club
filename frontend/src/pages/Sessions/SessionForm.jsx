...
const SessionForm = ({ mode = "create" }) => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      try {
        const res = await api.get("/members?active=true");
  ...
  // ===== Calcular fim esperado automaticamente =====
  useEffect(() => {
    setExpectedEndTime(calculateExpectedEnd(startTime, plannedMinutes));
  ...
  useEffect(() => {
    ...
    const loadSession = async () => {
      try {
        ...
        setSelectedMemberId(s.member_id ?? "");
        setSelectedMemberName(s.member_name ?? "");
        setSelectedClientId(s.client_id ?? "");
        setSelectedClientName(s.client_name ?? "");
        ...
        setNotes(s.notes ?? "");
        setStartTime(toLocalDatetime(s.start_time));
        setPlannedMinutes(s.planned_minutes ?? 60);
        setExpectedEndTime(
          s.expected_end_time
            ? toLocalDatetime(s.expected_end_time)
            : calculateExpectedEnd(s.start_time, s.planned_minutes)
        );
        setPlannedAmount(s.planned_amount ?? 0);
  ...
  // ===== Carregar clientes ativos para retorno =====
  useEffect(() => {
      ...
      <Form onSubmit={handleSubmit}>
        ...
        {visitType === "first" && (
          <>
            ...
            <FormGroup>
              ...
              <FormInput value={contact} onChange={(e) => setContact(e.target.value)} />
            </FormGroup>
            <FormGroup>
        ...
        <FormGroup>
          ...
          <div className="duration-quick-buttons">
            {[30, 60, 90, 120].map((m) => (
              <Button key={m} type="button" variant="secondary" onClick={() => applyQuickDuration(m)}>
                {m} min
              </Button>
            ))}
          </div>
        ...
        <FormGroup>
          ...
          <FormInput type="datetime-local" value={expectedEndTime} disabled />
        </FormGroup>
